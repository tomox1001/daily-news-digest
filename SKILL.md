...
## 5つの固定セクション（この順番を維持する）

| # | セクション | カード目安 | アイコン | グラデーション |
|---|-----------|----------|---------|-------------|
| 1 | 企業・開発組織のAI活用 最新事例 | 2-3 | 🏢 | gradient-3 |
| 2 | AI BPO — アウトソーシング革命 | 2 | 🏭 | gradient-2 |
| 3 | Product Hunt トレンド — 注目プロダクト | 2 | 🚀 | gradient-4 |
| 4 | AI・テクノロジー 最新ニュース | 3-5 | 🤖 | gradient-1 |
| 5 | 政治・国際動向 ｜ LMビジネス視点 | 2-3 | 🌏 | gradient-2 |

セクション順は意図的にこの順番に設定されている。企業のAI活用事例が最も実務に近い情報であるため先頭に置き、
...
ユーザーのoutputディレクトリに保存し、`computer://` リンクで即時プレビュー可能にする。
...
- `news-digest-index.html` — 全ダイジェストのアーカイブインデックス（同じoutputディレクトリに配置）
